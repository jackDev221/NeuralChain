...
func (p *peer) Info() *neut.PeerInfo {
	return &neut.PeerInfo{
		Version:    p.version,
		Difficulty: p.Td(),
...
func (p *peer) Handshake(td *big.Int, head common.Hash, headNum uint64, genesis common.Hash, server *LesServer) error {
	...
	if server != nil {
		if !server.onlyAnnounce {
			...
			// If local neuralChain node is running in archive mode, advertise ourselves we have
...
// the Light NeuralChain sub-protocol.
type peerSet struct {
