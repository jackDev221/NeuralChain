...
type LesServer struct {
	...
	archiveMode bool // Flag whether the neuralChain node runs in archive mode.
...
func NewLesServer(neut *neut.NeuralChain, config *neut.Config) (*LesServer, error) {
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(neut.BlockChain().Genesis().Hash(), pv)
	}

	srv := &LesServer{
		lesCommons: lesCommons{
			config:           config,
			chainDb:          neut.ChainDb(),
			iConfig:          light.DefaultServerIndexerConfig,
			chtIndexer:       light.NewChtIndexer(neut.ChainDb(), nil, params.CHTFrequency, params.HelperTrieProcessConfirmations),
			bloomTrieIndexer: light.NewBloomTrieIndexer(neut.ChainDb(), nil, params.BloomBitsBlocks, params.BloomTrieFrequency),
			protocolManager:  pm,
		},
		archiveMode:  neut.ArchiveMode(),
		quitSync:     quitSync,
		lesTopics:    lesTopics,
	...
	srv.costTracker, srv.minCapacity = newCostTracker(neut.ChainDb(), config, requestLogger)
	...
	srv.chtIndexer.Start(neut.BlockChain())
	return srv, nil
}
