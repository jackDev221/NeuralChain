...
func TestStartingTendermint(t *testing.T) {
	...
	pm1, err := neut.NewTestProtocolManagerWithConsensus(tbe1)
	...
	assert.NoError(t, neut.RegisterNewPeer(pm1, p1))
	assert.NoError(t, neut.RegisterNewPeer(pm1, p2))
	...
	go func() {
		err := p1.Handshake(neut.DefaultConfig.NetworkId, big.NewInt(0), headHash, genesisHash)
		assert.NoError(t, err)
	}()
	go func() {
		err := p2.Handshake(neut.DefaultConfig.NetworkId, big.NewInt(0), headHash, genesisHash)
		assert.NoError(t, err)
	}()
