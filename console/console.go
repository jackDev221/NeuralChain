...
import (
	...
	"github.com/lvbin2012/NeuralChain/internal/jsre"
	"github.com/lvbin2012/NeuralChain/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute NeuralChain requests through
...
func (c *Console) init(preload []string) error {
	...
	flatten := "var neut = web3.neut; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines neut.getBalance(neut.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
func (c *Console) Welcome() {
	message := "Welcome to the Gnc JavaScript console!\n\n"

	// Print some generic Gev metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		try {
			message += "coinbase: " + neut.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + neut.blockNumber + " (" + new Date(1000 * neut.getBlock(neut.blockNumber).timestamp) + ")\n";
		try {
