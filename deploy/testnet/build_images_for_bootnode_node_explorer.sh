...
BASEDIR=$(dirname "$0")
BUILDER_REPOSITORY="kybernetwork/neuralChain-builder"
...
then
  ...
  git clone --single-branch -b develop --single-branch git@github.com:NeuralChainLabs/NeuralChain.git "$BASEDIR"/builder/project
else
  echo "--- Cloning NeuralChain from tag $version ..."
  git clone -b "$version" git@github.com:NeuralChainLabs/NeuralChain.git "$BASEDIR"/builder/project
fi
...
sudo docker cp "$TempBuilderContainer":/neuralChain/gev "$BASEDIR"/nodes/bin/
sudo docker cp "$TempBuilderContainer":/neuralChain/bootnode "$BASEDIR"/bootnode/bin/
...
if [[ "$buildExplorer" == "y" ]]; then
  ...
  git clone git@github.com:NeuralChainLabs/explorer.git "$BASEDIR"/explorer/web
