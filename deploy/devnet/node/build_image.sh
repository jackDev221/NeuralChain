...
BASEDIR=$(dirname "$0")
git clone --single-branch -b develop --single-branch git@github.com:NeuralChainLabs/NeuralChain.git "$BASEDIR"/project
...
docker build -f dockerfiles/node/Dockerfile -t kybernetwork/NeuralChain:1.0.1-dev .
popd
