...
var (
	...
	retestethCommand = cli.Command{
		...
		Name:        "retesteth",
		Usage:       "Launches gnc in retesteth mode",
		ArgsUsage:   "",
		Flags:       []cli.Flag{rpcPortFlag},
		Category:    "MISCELLANEOUS COMMANDS",
		Description: `Launches gnc in retesteth mode (no database, no network, only retesteth RPC interface)`,
	}
)
...
func (api *RetestethAPI) ClientVersion(ctx context.Context) (string, error) {
	return "Gev-" + params.VersionWithCommit(gitCommit, gitDate), nil
}
...
func retesteth(ctx *cli.Context) error {
	...
	listener, _, err := rpc.StartHTTPEndpoint(httpEndpoint, rpcAPI, []string{"test", "neut", "debug", "web3"}, cors, vhosts, rpc.DefaultHTTPTimeouts)
	if err != nil {
