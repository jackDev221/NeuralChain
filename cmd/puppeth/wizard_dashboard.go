...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"neutstats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "neutstats":
				if infos, err := checkNeutstats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have neutstats running, ask whether to make the secret public or not
	if w.conf.neutstats != "" {
		fmt.Println()
		fmt.Println("Include neutstats secret on dashboard (y/n)? (default = yes)")
