...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"evrstats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "evrstats":
				if infos, err := checkEvrstats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have evrstats running, ask whether to make the secret public or not
	if w.conf.evrstats != "" {
		fmt.Println()
		fmt.Println("Include evrstats secret on dashboard (y/n)? (default = yes)")
