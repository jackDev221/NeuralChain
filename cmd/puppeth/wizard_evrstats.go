...
func (w *wizard) deployEvrstats() {
	...
	// Retrieve any active evrstats configurations from the server
	infos, err := checkEvrstats(client, w.network)
	if err != nil {
		infos = &evrstatsInfos{
			port:   80,
			host:   client.server,
	...
	fmt.Println()
	fmt.Printf("Which port should evrstats listen on? (default = %d)\n", infos.port)
	infos.port = w.readDefaultInt(infos.port)

	// Figure which virtual-host to deploy evrstats on
	if infos.host, err = w.ensureVirtualHost(client, infos.port, infos.host); err != nil {
		log.Error("Failed to decide on evrstats host", "err", err)
		return
	}
	// Port and proxy settings retrieved, figure out the secret and boot evrstats
	fmt.Println()
	if infos.secret == "" {
	...
	// Try to deploy the evrstats server on the host
	nocache := false
	if existed {
		fmt.Println()
		fmt.Printf("Should the evrstats be built from scratch (y/n)? (default = no)\n")
	...
	if out, err := deployEvrstats(client, w.network, infos.port, infos.secret, infos.host, trusted, infos.banned, nocache); err != nil {
		log.Error("Failed to deploy evrstats container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
