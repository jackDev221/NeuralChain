...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for evrstats availability")
	if infos, err := checkEvrstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["evrstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["evrstats"] = infos.Report()
		evrstats = infos.config
	}
