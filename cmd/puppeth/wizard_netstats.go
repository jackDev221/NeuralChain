...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for neutstats availability")
	if infos, err := checkNeutstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["neutstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["neutstats"] = infos.Report()
		neutstats = infos.config
	}
