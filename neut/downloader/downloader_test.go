...
type downloadTester struct {
	...
	stateDb neutdb.Database // Database used by the tester for syncing from peers
	peerDb  neutdb.Database // Database of the peers containing all data
...
func testSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	checkProgress(t, tester.downloader, "pristine", neuralChain.SyncProgress{})
	...
	<-starting
	checkProgress(t, tester.downloader, "initial", neuralChain.SyncProgress{
	...
	<-starting
	checkProgress(t, tester.downloader, "completing", neuralChain.SyncProgress{
	...
	progress <- struct{}{}
	pending.Wait()
	checkProgress(t, tester.downloader, "final", neuralChain.SyncProgress{
		StartingBlock: uint64(chain.len()/2 - 1),
...
func checkProgress(t *testing.T, d *Downloader, stage string, want neuralChain.SyncProgress) {
	// Mark this method as a helper to report errors at callsite, not in here
	t.Helper()
...
func testForkedSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	<-starting
	checkProgress(t, tester.downloader, "forking", neuralChain.SyncProgress{
