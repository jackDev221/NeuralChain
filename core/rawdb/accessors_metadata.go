...
func ReadDatabaseVersion(db neutdb.KeyValueReader) *uint64 {
	var version uint64
...
func WriteDatabaseVersion(db neutdb.KeyValueWriter, version uint64) {
...
func ReadChainConfig(db neutdb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db neutdb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
func ReadPreimage(db neutdb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(preimageKey(hash))
	return data
}

// WritePreimages writes the provided set of preimages to the database.
func WritePreimages(db neutdb.KeyValueWriter, preimages map[common.Hash][]byte) {
