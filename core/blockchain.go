...
type BlockChain struct {
	...
	db     neutdb.Database // Low level persistent database to store final content in
...
func (bc *BlockChain) SetHead(head uint64) error {
	...
	updateFn := func(db neutdb.KeyValueWriter, header *types.Header) {
	...
	delFn := func(db neutdb.KeyValueWriter, hash common.Hash, num uint64) {
	...
	if bc.cacheConfig.TrieDirtyDisabled {
		...
		if current := block.NumberU64(); current > TriesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - neutdb.IdealBatchSize)
			}
