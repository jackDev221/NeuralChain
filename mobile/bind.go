...
func (s *signer) Sign(addr *Address, unsignedTx *Transaction) (signedTx *Transaction, _ error) {
	sig, err := s.sign(types.BaseSigner{}, addr.address, unsignedTx.tx)
	if err != nil {
		return nil, err
	}
...
func DeployContract(opts *TransactOpts, abiJSON string, bytecode []byte, client *NeuralChainClient, args *Interfaces) (contract *BoundContract, _ error) {
...
func BindContract(address *Address, abiJSON string, client *NeuralChainClient) (contract *BoundContract, _ error) {
	parsed, err := abi.JSON(strings.NewReader(abiJSON))
	if err != nil {
		return nil, err
	}
