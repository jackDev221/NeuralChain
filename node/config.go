...
import (
	...
	"github.com/lvbin2012/NeuralChain/accounts/external"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gnc is "gnc". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gev", keep that.
	if name == "gnc" || name == "gnc-testnet" {
		name = "Gnc"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gnc" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gnc 0.0.1 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGethResourceWarning, "Using deprecated resource file %s, please move this file to the 'gnc' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
