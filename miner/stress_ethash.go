...
func main() {
	...
	for {
		...
		tx, err := types.SignTx(types.NewTransaction(nonces[index], crypto.PubkeyToAddress(faucets[index].PublicKey), new(big.Int), 21000, big.NewInt(100000000000+rand.Int63n(65536)), nil), types.BaseSigner{}, faucets[index])
		if err != nil {
			panic(err)
		}
...
func makeMiner(genesis *core.Genesis) (*node.Node, error) {
	...
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		return neut.New(ctx, &neut.Config{
			Genesis:         genesis,
			...
			GPO:             neut.DefaultConfig.GPO,
			Ethash:          neut.DefaultConfig.Ethash,
			Miner: Config{
