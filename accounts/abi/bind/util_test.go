...
var addr, _ = common.EvryAddressStringToAddressCheck("ENTHVCWQS29XPhqygjQ5fyNYjYLKFDLJ5J")
...
func TestWaitDeployed(t *testing.T) {
	for name, test := range waitDeployedTests {
		...
		tx, _ = types.SignTx(tx, types.BaseSigner{}, testKey)
		...
		case <-mined:
			...
			if address != test.wantAddress {
				t.Errorf("test %q: unexpected contract address %s", name, address.String())
			}
