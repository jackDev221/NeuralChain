...
type Wallet struct {
	...
	deriveChain     evrynetNode.ChainStateReader // Blockchain state reader to discover used account with
...
// chain state reader.
func (w *Wallet) SelfDerive(bases []accounts.DerivationPath, chain evrynetNode.ChainStateReader) {
	w.lock.Lock()
	defer w.lock.Unlock()
