...
type Wallet struct {
	...
	deriveChain     neuralChain.ChainStateReader // Blockchain state reader to discover used account with
...
// chain state reader.
func (w *Wallet) SelfDerive(bases []accounts.DerivationPath, chain neuralChain.ChainStateReader) {
	w.lock.Lock()
	defer w.lock.Unlock()
