...
// Package accounts implements high level NeuralChain account management.
package accounts
...
// Account represents an NeuralChain account located at a specific location defined
...
type Wallet interface {
	...
	// chain state reader.
	SelfDerive(bases []DerivationPath, chain neuralChain.ChainStateReader)
	...
	// by the NeuralChain prefix scheme
	// It looks up the account specified either solely via its address contained within,
	// or optionally with the aid of any location metadata from the embedded URL field.
	//
	// If the wallet requires additional authentication to sign the request (e.g.
...
//   keccak256("\x19NeuralChainNode Signed Message:\n"${message length}${message}).
//
...
func TextAndHash(data []byte) ([]byte, string) {
	msg := fmt.Sprintf("\x19NeuralChainNode Signed Message:\n%d%s", len(data), string(data))
